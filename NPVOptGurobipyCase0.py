...
For the 0th case to compare losses
"""
...
x_p = np.arange(years)
liquid_price = np.interp(x_quarters,x_p,price_lng[:int(years)])*0.137381*0 #MMBTU/GAL
...
for i in range(len(flowrate)):
    ...
    D_Cap_Cost.append(costing['Total Plant Cost (x $1000)'][19]*1000) #Downstream
...
def NPV_t(m,t):
    ...
    if t<120:
        depreciation = (
            ...
            #+D_Cap_Cost[0] / 120 * gp.quicksum(well_open[tau,w] for w in W for tau in range(t)) #Downstream depreciation
    ...
    PBT = (
        ...
        -gp.quicksum(F_Op_Cost_Flares*well_open[tau,w] for w in W for tau in range(t+1))*wells_per_reactor #operating cost of flares
    ...
    return net_cash#discounted_cash_flow(t)*net_cash
...
plt.tight_layout()
if save:
    plt.savefig(f'Figs/ReactorDeployment__{quarters}q_case{Case}_marketprice_wpr{wells_per_reactor}_we',dpi=500)
plt.show()


# Save all arrays to a single .npz file
if save:
    sp = f"gurobi_results_{quarters}q_case0_wpr{wells_per_reactor}.npz"
    np.savez(sp,
            inv=inv_np,
            buy=buy_np,
            x=x_np,
